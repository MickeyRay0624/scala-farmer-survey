...
更新日期：2026-09-02  
问卷版本：`scala-farmer-survey-2026-09-02-v1`
...
5. 离线时可以继续填写和保留本机草稿，但必须恢复网络后才能提交。

这一方案不需要新建数据库，也不需要在公开仓库保存个人数据，但后续分析时必须先拆解 Google Sheet 中的 JSON 单元格。
...
8. Google Sheet 中以 JSON 保存详细答案是否满足当前分析流程，还是要改建逐题原生字段的后端？
